...
var _ = function(elem, attr) { return new DOMBuilder(elem, attr); };
...
document.body.appendChild(
    _('div', { 'class':'location_select_control' }).child(
        _('a', { 'href':'', 'class':'location_select_label' }).child([
            ...
            _('label').innerHTML('This is another label')
        ])
    ).asDOM()
);</pre>

See <code>examples.htm</code> for more complex examples.
...
* <code>replace</code> - {Boolean} (Required) Whether this new value should replace the existing value. Defaults to append (false).
